...
export function AuthShell({ children, headline, subhead }) {
    return (
        <div className="min-h-screen flex bg-(--bg) p-3 sm:p-4 gap-0 lg:gap-4">
            {/* Left — form */}
    ...
    return (
        <div className="hidden lg:block flex-1 relative rounded-[28px] overflow-hidden isolate">
            ...
            <div
                ...
                style={{
                    background:
                        "linear-gradient(140deg, #0b2b27 0%, #0f766e 38%, #0c3b36 72%, #071a18 100%)",
                }}
            />

            {/* Animated radial glows */}
            <motion.div
                className="absolute -top-32 -right-32 w-130 h-130 rounded-full pointer-events-none"
                style={{
                    background:
            ...
            />
            <motion.div
                className="absolute -bottom-40 -left-32 w-115 h-115 rounded-full pointer-events-none"
                style={{
                    background:
            ...
            <div className="relative z-10 h-full flex flex-col pt-10 xl:pt-16">
                ...
                >
                    ...
                    <h2
                        className="font-serif text-[44px] xl:text-[60px] leading-[1.02] text-white mt-8 max-w-135"
                        style={{
                            fontStyle: "italic",
                            fontWeight: 500,
        ...
        <div>
            ...
            <div className="relative">
                <input
                    ...
                    } rounded-2xl border border-(--border) bg-(--surface) text-[15px] text-(--ink) placeholder:text-(--ink-muted)/60 outline-none transition-all duration-200 focus:border-(--accent)/40 focus:ring-4 focus:ring-(--accent)/10`}
                />
                {Icon && (
                    <div className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-(--ink-muted)/55 peer-focus:text-(--accent-strong) peer-not-placeholder-shown:text-(--accent-strong) transition-colors">
...
export function AuthErrorBanner({ children }) {
    ...
    return (
        <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            className="text-xs text-(--danger) bg-(--danger)/10 rounded-2xl px-4 py-2.5 leading-snug"
        >
            {children}
        </motion.div>
    );
